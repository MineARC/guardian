...
function poll_aura() {
  var request_options = {url: 'http://172.17.0.5/', proxy: ''};

  var request_options_ext = {url: 'http://172.17.0.6/', proxy: ''};

  var request_options_node = {url: 'http://172.17.0.7/api', proxy: ''};
  ...
  request.get(request_options_node, function(err, res, body) {
    if (!err && (res.statusCode == 200 || res.statusCode == 304)) {
      processPageNode(body);
    }
  });
...
function processPageNode(data) {
  var assets = JSON.parse(data).assets;
  exports.version_ext = 'node';

  for (node in assets) {
    for (gas in assets[node].readings) {
      if (gas.toUpperCase() in aura_ext_data) {
        aura_ext_data[gas.toUpperCase()].value =
            assets[node].readings[gas].value.toFixed(1);
        if (gas == 'co2')
          aura_ext_data[gas.toUpperCase()].value =
              assets[node].readings[gas].value.toFixed(2);
        timestamps_ext[gas.toUpperCase()] = Date.now();
      }
    }
  }
}
